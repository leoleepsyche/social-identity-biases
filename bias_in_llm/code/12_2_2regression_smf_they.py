...
def run_logit_analysis(llm: str, sentiment_classifier: str, type: str):
    path = f'.\\data\\all_data_berttopic_stm_control_sentiment_splitthey.csv'
    df = pd.read_csv(path)
    df = df[df['model']==llm]
    df = df[df['berttopic'].notnull()]
    df = df[df['sentiment_aliyun'].isin({'positive', 'negative', 'neutral'})]
    df = df[df['group'].isin({'we', 'they'})]
...
sentiment_list = ['erlangshen', 'aliyun']


batch_logit_analysis(llm_list, sentiment_list, '.\\data\\logistic_result_they.csv')